...
public class PrimeLab {
    ...
    // 3. Modular exponentiation for BigInteger
    private static BigInteger modPow(BigInteger base, BigInteger exp, BigInteger mod) {
        return base.modPow(exp, mod);
    }

    // 4. Miller-Rabin probabilistic test O(k log^3 n)
    public static boolean millerRabin(long n, int k) {
        ...
        for (int i = 0; i < k; i++) {
            BigInteger a = BigInteger.valueOf(
                    ThreadLocalRandom.current().nextLong(2, n - 1)
            );
    ...
    public static void main(String[] args) {
        ...
        if (td!= mr) {
            System.out.println("Note: Probabilistic test disagrees. May be a pseudoprime.");
        }
